...
const defaults = {
  ...
  settings: {
    ...
    maxNights: 14,
    defaultNights: 9,
    firstPersonCostPerNight: 99.98,
    extraPersonCostPerNight: 80.00,
  },

  content: {
    title: "My BRB subscription",
...
const calculatePrice = (people, nights, firstPersonCostPerNight, extraPersonCostPerNight) => {
  let total = nights * firstPersonCostPerNight; // One person
  if (--people) {
    total += people * nights * extraPersonCostPerNight;
  }
  return (total / 12).toFixed(2);
};

const query = graphql`{
  contentfulConfigurationItem(name: {eq: "Pricing calculator"}) {
    name
    data {
      settings {
        minPeople
        ...
        defaultNights
        stepNights
        firstPersonCostPerNight
        extraPersonCostPerNight
      }
      content {
        title
        text
...
const SubscriptionCalculator = () => {
  ...
  const { settings, content } = defaults;
  ...
  const [nights, setNights] = useState(settings.defaultNights);

  const handleUpdatePeople = (e) => {
    setPeople(e.target.value);
  }

  const handleUpdateNights = (e) => {
      ...
      <Content>
        ...
        <Selectors>
          ...
          <Selector
            ...
            min={settings.minPeople}
            max={settings.maxPeople}
            step={1}
            defaultValue={people}
            suffixes={{ singular: 'traveller', plural: 'travellers' }}
            changeHandler={handleUpdatePeople}
          />
        ...
        <SubscriptionDetails>
          ...
          <Price className="simple-animate-in">
            <span>
              £{calculatePrice(people, nights, settings.firstPersonCostPerNight, settings.extraPersonCostPerNight)}
            </span>&nbsp;/&nbsp;month
          </Price>
        </SubscriptionDetails>
