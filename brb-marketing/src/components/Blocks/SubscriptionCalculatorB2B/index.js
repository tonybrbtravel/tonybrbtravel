...
const query = graphql`{
  contentfulConfigurationItem(name: {eq: "[B2B] Pricing calculator"}) {
    name
    data {
      settings {
        employeeOptions
...
const processQueryResult = (queryData) => {
  const { settings, content } = queryData.contentfulConfigurationItem.data;
...
const SubscriptionCalculator = () => {
  ...
  if (queryResult.contentfulConfigurationItem) {
    ...
    Object.assign(settings, processedData.settings);
    Object.assign(content, processedData.content);
  }

  const calculatePrice = (employees, trips) => {
    const employeeIndex = settings.employeeOptions.indexOf(employees);
    const tripIndex = settings.tripOptions.indexOf(trips);
    const tripArrayLength = settings.tripOptions.length;
    if (employeeIndex !== -1 && tripIndex !== -1 ) {
      return settings.costs[employeeIndex * tripArrayLength + tripIndex];
    }
    return 'error';
  };

  const subscribeLink = (activeEnv === 'development') ?
    content.cta.href_development || content.cta.href :
    content.cta.href;

  const [employeeOption, setEmployeeOption] = useState(settings.defaultEmployeeOption);
  const [tripOption, setTripOption] = useState(settings.defaultTripOption);

  const handleUpdateEmployeeOption = (e) => {
    setEmployeeOption(e.target.value);
  }

  const handleUpdateTripOption = (e) => {
    setTripOption(e.target.value);
  }
      ...
      <Content>
        ...
        <Selectors>
          ...
          <Selector
            ...
            options={settings.employeeOptions}
            defaultValue={employeeOption}
            changeHandler={handleUpdateEmployeeOption}
          />

          <Selector
            className="simple-animate-in"
            icon={nightsSelectSvg}
            question="How many 2-night trips per year?"
            options={settings.tripOptions}
            defaultValue={tripOption}
            changeHandler={handleUpdateTripOption}
          />
        ...
        <SubscriptionDetails>
          ...
          <Price className="simple-animate-in">
            <span>
              £{calculatePrice(employeeOption, tripOption, settings.firstPersonCostPerNight, settings.extraPersonCostPerNight)}
            </span> per&nbsp;employee per&nbsp;month
          </Price>
        </SubscriptionDetails>
        ...
        <List>
          ...
          <ul className="simple-animate-in">
            {content.list.items.map((item, index) => <li key={index}>{item}</li>)}
          </ul>
        </List>
        {/* End list */}
